...
import { Webhook, Trash2, Key, RefreshCw } from "lucide-react"
...
export function DeveloperConsoleClient() {
  ...
  const fetchLogs = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/v1/developer/webhooks/logs`, {
        headers: { "Authorization": `Bearer ${localStorage.getItem("token")}` }
      });
  ...
  const createWebhook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUrlInput("");
        setEventInput("scan.completed");
        toast.success("Webhook created!", {
          description: `Your secret HMAC token is: ${data.secret_token}. Save this now, it won't be shown again!`,
          duration: 10000
        });
      } else {
