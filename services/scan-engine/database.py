...
if not _raw_url:
    # No DATABASE_URL configured (local dev / quick BYOK smoke test). Fall back
    # to a local SQLite file so the API boots with zero external infrastructure.
    logger.warning("DATABASE_URL not set - falling back to local SQLite (aegisml.db).")
    _raw_url = "sqlite+aiosqlite:///./aegisml.db"

# Normalize driver prefixes
if _raw_url.startswith("postgres://"):
    final_url = _raw_url.replace("postgres://", "postgresql+asyncpg://", 1)
elif _raw_url.startswith("postgresql://") and "+asyncpg" not in _raw_url:
    final_url = _raw_url.replace("postgresql://", "postgresql+asyncpg://", 1)
elif _raw_url.startswith("sqlite://") and "+aiosqlite" not in _raw_url:
    final_url = _raw_url.replace("sqlite://", "sqlite+aiosqlite://", 1)
else:
    final_url = _raw_url
...
DATABASE_BACKEND = "sqlite" if _is_sqlite else "postgresql"
...
if _is_sqlite:
    # asyncpg-only args (ssl, pool sizing) are invalid for aiosqlite.
...
class ScanRecord(Base):
    __tablename__ = "scans"
    __table_args__ = (
        # Supports an explicit future public-sharing flow without making scans
        # public by default.
        Index("ix_scans_public_created", "is_public", "created_at"),
        # Serves the /stats risk_level aggregations
    ...
    # BigInteger: pro-tier uploads reach 5 GB, which overflows a 32-bit INT.
