...
Previously this returned a single hardcoded canned answer (the exact same
text, the exact same `related_patterns: ["PKL-001", "PKL-012"]`) regardless
of what the user actually asked. This now grounds the answer in the real
300+ pattern library and asks an actual model for a real response.
"""
import json
import os
import re
...
async def natural_language_query(question: str, patterns: list[dict] | None = None) -> dict:
    ...
    context_lines = "\n".join(
        f"- {p.get('id')}: {p.get('name')} [{p.get('severity')}, CVSS {p.get('cvss')}] — {p.get('description')}"
        for p in related
    )
    prompt = (
    ...
    try:
        ...
        return {
            "question": question,
            "answer": (
                f"AI analysis failed ({str(e)[:120]}). Based on keyword matching, "
