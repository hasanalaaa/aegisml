...
# Previously this was `require_role()` returning a hardcoded fake admin
# identity ({"user_id": uuid4(), "role": "admin"}) WITHOUT EVER CHECKING A
# TOKEN — meaning the entire enterprise surface (audit logs, custom threat
# rules, member management) was completely unauthenticated and unauthorized.
# This now performs real JWT-backed authentication via get_current_user and
# enforces the required role.
def require_role(required_role: str):
    async def role_dependency(
        ...
        # 'enterprise' plan holders and explicit 'admin' role users are
        # treated as org admins for their own org space.
        is_admin = current_user.role == "admin" or current_user.plan == "enterprise" or current_user.current_plan == "enterprise"
