...
async def get_dataset(
    ...
    """Return a real anonymized dataset built from actual scan history."""
    ...
    if format in ("csv", "parquet"):
        ...
        # CSV needs scalar cells; serialize the list column
        flat = [{**row, "threat_categories": ";".join(row["threat_categories"])} for row in data] if format == "csv" else data
        df = pd.DataFrame(flat) if flat else pd.DataFrame(
            columns=["file_type", "verdict", "threat_categories", "cvss_score", "entropy_score", "timestamp"]
        )
        if format == "csv":
            return Response(
        ...
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=aegisml_dataset.parquet"},
        )

    raise HTTPException(status_code=400, detail="Invalid format. Use json, csv, or parquet.")
