...
# --- Schemas ---
class ModelURLPayload(BaseModel):
    model_config = ConfigDict(protected_namespaces=())


class ReviewCreate(ModelURLPayload):
    model_url: str
    rating: int
    comment: Optional[str] = None

class ReviewResponse(ModelURLPayload):
    id: int
    model_url: str
    rating: int
...
class BookmarkCreate(ModelURLPayload):
    model_url: str

class BookmarkResponse(ModelURLPayload):
    id: int
    model_url: str
    created_at: datetime
...
async def get_leaderboard(limit: int = Query(20, ge=1, le=100), db: AsyncSession = Depends(get_db)):
    """Rank only scans whose owners explicitly opted into public sharing."""
    stmt = (
        select(
