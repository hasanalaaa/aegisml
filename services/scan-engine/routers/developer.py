...
VALID_EVENTS = frozenset({"scan.completed", "threat.critical", "all"})

def _assert_public_url(url: str) -> None:
    """SSRF guard: reject webhook URLs resolving to private/internal networks."""
    import ipaddress
    import socket
    ...
    for info in infos:
        ...
        if (
            ...
            raise HTTPException(
                status_code=400,
                detail="Webhook URL must resolve to a public address",
            )
...
async def create_webhook(
    ...
    if invalid:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown event(s): {', '.join(invalid)}. Valid: {', '.join(sorted(VALID_EVENTS))}",
        )
    _assert_public_url(str(webhook.url))
...
async def delete_webhook(
    ...
    """Delete a webhook subscription (ownership enforced)."""
