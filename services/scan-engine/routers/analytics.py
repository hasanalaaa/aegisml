...
async def export_scan_csv(scan_id: str, db: AsyncSession = Depends(get_db)):
    ...
    system import. Complements the existing PDF report endpoint above."""
...
async def export_all_scans_csv(
    ...
    df = pd.DataFrame(rows) if rows else pd.DataFrame(
        columns=["scan_id", "filename", "file_extension", "file_size",
                 "risk_level", "risk_score", "threat_count", "source_type", "created_at"]
    )
